<?php
	...
	class AdPostsController extends AppController {
		...
		public $uses = array('ItemCategory', 'Brand', 'Role', 'AutoModel', 'AdPost', 'ItemPhoto', 'ItemType');
		...
		AppController::beforeFilter();
			...
			$this->Auth->allow("index", 'edit_post', "get_item_categories", 'get_brands', 'get_models', 'get_cities', 'get_states', 'get_areas');
		...
		public function index() {
			...
			if ($this->request->is('post')) {
				...
				$final_aray['created'] = $final_date;
				$final_aray['modified'] = $final_date;
				...
				if($this->AdPost->save($final_aray)) {
					...
					$this -> redirect(array('controller' => 'categories', 'action' => 'product_detail', 'ad_1433864959_55770aff-6e5c-450b-a6fe-2c2752d40771'));
				}
				else {					
					$this->Session->setFlash('Sorry, an error occurred.', 'default', array('class' => 'alert alert-danger') , 'error');
                	$this -> redirect(array('controller' => 'ad_posts', 'action' => 'index'));
		...
		public function get_brands($id=null) {
			...
			$brands = $this->Brand->find('all', array(
					'conditions' => array('Brand.item_type_id' => $id),
            ...
            foreach ($brands as $brand) {
                $content .= '<option value="'.$brand['Brand']['id'].'" >'.$brand['Brand']['name'].'</option>';
            }
  ...
  //       	echo $content;		
		...
		// 		$tmp_array['City']['name'] = $value['doc']['name'];
		// 		$tmp_array['City']['region'] = $value['doc']['region1'];
